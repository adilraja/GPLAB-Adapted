function [fitness,resultind,slope,intercept,state]=calcfitness(ind,params,data,state)
...
if isempty(state.keepevals) % this means keepevals has not be initialized (on purpose)
   [fitness,resultind,slope,intercept]=feval(params.calcfitness,ind,params,data,state.terminals,state.varsvals);
   return;
end
...
f=find(strcmp(state.keepevals.inds,ind.str));%Adil adds str to last argument
