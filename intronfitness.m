function [fitness,resultind,slope,intercept,state]=intronfitness(ind,params,data,state)
...
if isempty(state.keepevals) % this means keepevals has not be initialized (on purpose)
   [fitness,resultind,slope,intercept]=feval('mse_adil',ind,params,data,state.terminals,state.varsvals);%Adil uses mse_adil here
   return;
end
