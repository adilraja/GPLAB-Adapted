function [fitness,resultind,bCoeff,aCoeff]=rsquareFitness(ind,params,data,terminals,varsvals)
...
fitness=meandif;
%fitness=1-meandif/var(data.result); %higher fitness means better individual. From wiki
...
if(isnan(fitness) | isinf(fitness) | ~isreal(fitness))
    fitness=0;%Give a very small value to fitness so that the individual is removed:Adil
elseif(std(res)~=0)
    fitness=1-meandif/var(data.result);
else
    fitness=0;
end

if fitness>1
    printf(1,'hey fitness is greater');
end
...
if length(evolved) ~= length(target)
 ...
 b = 1; a=0; numerator=0;denominator=0;
     ...
     bcoeff=1;
 if denominator~=0     
     bcoeff=numerator/denominator;
 elseif  (meanEvolved == 0)  
     bcoeff = 1;
 else
     bcoeff = meanTarget / meanEvolved;
 end
