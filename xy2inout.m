...
if nx/4==ny || nx==ny % if dimensions match Adil
 ...
 %  xy=uniquenosort(xy);
  ...
  % result.example(1:nx,:)=x(1:nx,:);
   %result.result=y;%Adil deliberately commented this and all the above
   %lines for the sake of P.563 dimensionality reduction test. Should be
   %uncommentd afterwards.
   result.example=x;
   result.result=y;
