function [indid,indindex,popexpected,popnormfitness]=lexictour_adil(pop,params,state,nsample,toavoid)
...
if params.lowerisbetter
   ...
   betterfits(1,:)=repmat(min(tgroupfits(1,:)),1,params.tournamentsize);
   for(i=2:nsample)
         f_adil=tgroupfits(i,:)==betterfits(i-1,:);% find if the second pool has something with something equal to the min fitness of first pool?
         tempAdil=max(tgroupfits(i,:));%find the max fitness in the second pool
         f_adil=tempAdil.*f_adil;%fadil was only an array of zeros and ones, multiply it with the max fitness of the second pool
         tgroupfits(i,:)= tgroupfits(i,:)+f_adil;%Now add it to the tgroupfitness, result is that where ever the fitness of ind 
                                          %in 2nd pool is equal to the min fitness of first that fitness has been replaced with a fitness value much higher than the max fitness of this pool
         betterfits(i,:)=repmat(min(tgroupfits(i,:)),1,params.tournamentsize);% and here we go, we implemented a new method for tournamnet selection
         betterfits=Roundoff(betterfits,4);%Individuals with slightly (minutely) varying fitness values are considered to have the same fitness for the selection purposes
   end

else
   betterfits=repmat(max(tgroupfits,[],2),1,params.tournamentsize);
   betterfits(1,:)=repmat(max(tgroupfits(1,:)),1,params.tournamentsize);
   for(i=2:nsample)
         f_adil=tgroupfits(i,:)==betterfits(i-1,:);% find if the second pool has something with something equal to the max fitness of first pool?
         tempAdil=min(tgroupfits(i,:));%find the min fitness in the second pool
         f_adil=tempAdil.*f_adil;%fadil was only an array of zeros and ones, multiply it with the min fitness of the second pool
         tgroupfits(i,:)= tgroupfits(i,:)-f_adil;%Now add it to the tgroupfitness, result is that where ever the fitness of ind 
                                          %in 2nd pool is equal to the max fitness of first that fitness has been replaced with a fitness value much lower than the min fitness of this pool
         betterfits(i,:)=repmat(max(tgroupfits(i,:)),1,params.tournamentsize);% and here we go, we implemented a new method for tournamnet selection
         betterfits=Roundoff(betterfits,4);%Individuals with slightly (minutely) varying fitness values are considered to have the same fitness for the selection purposes
   end
end
