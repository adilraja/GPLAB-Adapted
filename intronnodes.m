...
%
%   Input arguments:
%      TREE - the tree to count introns (struct)%tree is replaced with ind
...
if nkids>0
  % slope=0;intercept=0;
   [ans,rootresult,slope,intercept,state]=intronfitness(tree2str(tree),params,data,state);%Adil changed calcfitness to intronfitness
      
   for i=1:nkids   
      [ans,kidresult{i},slope,intercept,state]=intronfitness(tree2str(tree.kids{i}),params,data,state);%Adil changed calcfitness to intronfitness
