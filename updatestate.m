...
if isempty(state.bestsofar) | ((params.lowerisbetter & pop(bestindex).fitness<state.bestsofar.fitness) | (~params.lowerisbetter & pop(bestindex).fitness>state.bestsofar.fitness))
   ...
   params.slope=state.bestsofar.slope;%adil's addition
   params.intercept=state.bestsofar.intercept;%adil's addition
end
...
if params.usetestdata
   ...
   for t=1:params.numvars
    ...
    params.slope=state.bestsofar.slope;params.intercept=state.bestsofar.intercept;
	[testfitness, testresult]=feval(params.calcfitness2,state.bestsofar,params,data.test,state.terminals,tmpstate.varsvals);%adil's addition. changed state.bestsofar.str to state.bestsofar
    state.bestsofar.testfitness=testfitness;
    state.bestsofar.testresult=testresult;
