...
if ~isempty(tree.kids)
    ...
    opText = tree.op;
    if(~isempty(findstr(opText,'my')))
        opText=opText(3:length(opText));
    end
    if(~isempty(findstr(opText,'koza')))
        opText=opText(5:length(opText));
    end
    text(tree.X, tree.Y, ['  ' opText], 'HorizontalAlignment', 'left', 'interpreter', 'none','FontSize',10)
else
    opText = tree.op;
    line(tree.X, tree.Y, 'marker', '.', 'markersize', 8)
    text(tree.X, tree.Y, opText, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'interpreter', 'none','FontSize',16)
end
